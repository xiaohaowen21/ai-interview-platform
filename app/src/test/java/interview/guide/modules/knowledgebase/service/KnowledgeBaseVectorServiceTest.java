...
class KnowledgeBaseVectorServiceTest {
    ...
    private void mockVectorSearchResult(List<Document> results) {
        when(vectorStore.similaritySearch(any(SearchRequest.class))).thenReturn(results);
    }

    private List<Document> executeSearch(String query, List<Long> knowledgeBaseIds, int topK) {
        return vectorService.similaritySearch(query, knowledgeBaseIds, topK, 0);
    }
    ...
    class SimilaritySearchTests {
        ...
        void testBasicSearchWithoutFilter() {
            ...
            mockVectorSearchResult(mockResults);

            // When
            List<Document> results = executeSearch(query, null, topK);
        ...
        void testSearchWithKnowledgeBaseIdFilterString() {
            ...
            // When
            List<Document> results = executeSearch(query, knowledgeBaseIds, topK);
        ...
        void testTopKLimit() {
            ...
            // When
            List<Document> results = executeSearch(query, List.of(1L), topK);
        ...
        void testSearchWithEmptyKnowledgeBaseIdList() {
            ...
            // When
            List<Document> results = executeSearch(query, emptyList, topK);
        ...
        void testSearchReturnsEmpty() {
            // Given
            String query = "不存在的内容";
            mockVectorSearchResult(List.of());

            // When
            List<Document> results = executeSearch(query, null, 10);
        ...
        void testFilteredResultsEmpty() {
            ...
            // When
            List<Document> results = executeSearch(query, knowledgeBaseIds, 10);
    ...
    class EdgeCaseTests {
        ...
        void testEmptyQuery() {
            ...
            // When
            List<Document> results = executeSearch(emptyQuery, null, 5);
        ...
        void testTopKZero() {
            ...
            // When
            List<Document> results = executeSearch(query, null, 0);
