...
public class RedisService {
    ...
    public void createStreamGroup(String streamKey, String groupName) {
        ...
        try {
            ...
            // 某些 Redis/客户端组合下，MKSTREAM 可能未稳定创建空流。
            // 通过写入一条引导消息确保 key 存在，再补建消费者组。
            if (isStreamNotExistsError(e) || isNoGroupError(e)) {
                try {
                    stream.add(StreamAddArgs.entry("__bootstrap__", "1"));
                    stream.createGroup(StreamCreateGroupArgs.name(groupName));
                    log.info("通过引导消息创建 Stream 消费者组: stream={}, group={}", streamKey, groupName);
                    return;
                } catch (Exception inner) {
                    if (isBusyGroupError(inner)) {
                        return;
                    }
                    log.warn("创建消费者组失败: stream={}, group={}, error={}", streamKey, groupName, inner.getMessage());
                    return;
                }
            }
    ...
    private boolean isStreamNotExistsError(Throwable throwable) {
        return containsMessage(throwable, "requires the key to exist");
    }
