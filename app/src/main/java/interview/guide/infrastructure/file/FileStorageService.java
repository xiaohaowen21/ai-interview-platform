...
public class FileStorageService {
     ...
     * 上传首页展示图片（管理员配置）
     */
    public String uploadLandingAsset(MultipartFile file) {
        return uploadFile(file, "landing-assets");
    }
    ...
    public StoredFile downloadPublicAsset(String fileKey) {
        if (fileKey == null || !fileKey.startsWith("landing-assets/")) {
            throw new BusinessException(ErrorCode.STORAGE_DOWNLOAD_FAILED, "不支持访问该文件");
        }
        if (!fileExists(fileKey)) {
            throw new BusinessException(ErrorCode.STORAGE_DOWNLOAD_FAILED, "文件不存在: " + fileKey);
        }
        ...
        try {
            GetObjectRequest getRequest = GetObjectRequest.builder()
                    ...
                    .key(fileKey)
                    .build();
            var objectBytes = s3Client.getObjectAsBytes(getRequest);
            ...
            log.error("下载公开资源失败: {} - {}", fileKey, e.getMessage(), e);
            throw new BusinessException(ErrorCode.STORAGE_DOWNLOAD_FAILED, "文件下载失败: " + e.getMessage());
        }
    }
    ...
    private String uploadFile(MultipartFile file, String prefix) {
        ...
        try {
            ...
            if (e.statusCode() == 404 || "NoSuchBucket".equalsIgnoreCase(e.awsErrorDetails().errorCode())) {
                ensureBucketExists();
                throw new BusinessException(
                    ErrorCode.STORAGE_UPLOAD_FAILED,
                    "上传暂时失败：文件存储空间正在初始化，请稍后重试。"
                );
            }
            throw new BusinessException(ErrorCode.STORAGE_UPLOAD_FAILED, "上传失败：文件暂时无法保存，请稍后重试。");
        }
    }
    ...
    public String getPublicAssetUrl(String fileKey) {
        return "/api/public/storage?key=" + URLEncoder.encode(fileKey, StandardCharsets.UTF_8);
    }
    ...
    public void ensureBucketExists() {
        try {
            ...
            if (e.statusCode() == 404 || "NoSuchBucket".equalsIgnoreCase(e.awsErrorDetails().errorCode())) {
                log.info("存储桶缺失，正在补建: {}", storageConfig.getBucket());
                CreateBucketRequest createRequest = CreateBucketRequest.builder()
                        .bucket(storageConfig.getBucket())
                        .build();
                s3Client.createBucket(createRequest);
                log.info("存储桶补建成功: {}", storageConfig.getBucket());
                return;
            }
    ...
    public record StoredFile(byte[] bytes, String contentType, String fileName) {
    }
}
